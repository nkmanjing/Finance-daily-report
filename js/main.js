        ...
        ${data.news.map((news, index) => `
          <div class="news-card p-5">
            <div class="flex items-start gap-4">
              <div class="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center flex-shrink-0 text-sm font-bold">
                ${index + 1}
              </div>
              <div class="flex-1 min-w-0">
                <h3 class="font-semibold text-white mb-2 truncate">${news.title}</h3>
                <p class="text-slate-400 text-sm leading-relaxed">${news.summary}</p>
                ${news.link ? `
                  <a href="${news.link}" target="_blank" class="inline-flex items-center mt-3 text-blue-400 text-sm hover:text-blue-300 transition-colors">
                    <span>查看原文</span>
